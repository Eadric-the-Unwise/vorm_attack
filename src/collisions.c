...
UINT8 npc_tile_change[10] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};                                    // FOR TILE CHANGE POINTERS WHEN 0X00 ARE NECESSARY
...
UINT8 collide_bullet(UINT8 bullet_spawn_x, UINT8 bullet_spawn_y) {  // CHECK WHICH BACKGROUND TILE THE BULLET COLLIDES WITH
    ...
    if ((current_tile >= 0x03) && (current_tile <= 0x0C)) {                        // IF COLLIDING TILE IS ANY OF THE ENEMY TILES
        ...
        } else if ((current_tile >= 0x17) && (current_tile <= 0x20)) {  // POINT TO LIGHT GREEN MOTHERSHIP
        ...
        if ((current_tile == MOTHERSHIPS_BOTTOMS) && (bulletx - tile_x >= 6)) {             // IF PLAYER HITS BOTTOM OF BOTH DARK GREEN MOTHERSHIPS
            set_bkg_tile_xy(index_X, index_Y, MOTHERSHIPS_BOTTOM_RIGHT_GREEN_BOTTOM_LEFT);  // MOTHERSHIPS_BOTTOM_RIGHT_GREEN_BOTTOM_LEFT
            set_bkg_tile_xy(index_X + 1, index_Y, MOTHERSHIP_BOTTOM_RIGHT_GREEN);           // MOTHERSHIP_BOTTOM_RIGHT_GREEN
            set_bkg_tile_xy(index_X, index_Y - 1, MOTHERSHIPS_TOP_RIGHT_GREEN_TOP_LEFT);    // MOTHERSHIPS_TOP_RIGHT_GREEN_TOP_LEFT
        ...
        if ((current_tile == MOTHERSHIP_BOTTOM_LEFT) && (bulletx - tile_x >= 2)) {
            ...
            set_bkg_tile_xy(index_X, index_Y, MOTHERSHIP_BOTTOM_LEFT_GREEN);
            set_bkg_tile_xy(index_X, index_Y - 1, MOTHERSHIP_TOP_LEFT_GREEN);

            return 0x01U;
        } else if ((current_tile == MOTHERSHIPS_BOTTOMS) && (bulletx - tile_x <= 3)) {      // IF PLAYER HITS LEFT SIDE OF BOTH DARK GREEN MOTHERSHIPS BOTTOMS
            set_bkg_tile_xy(index_X, index_Y - 1, MOTHERSHIPS_TOP_RIGHT_TOP_LEFT_GREEN);    // MOTHERSHIPS_TOP_RIGHT_TOP_LEFT_GREEN
            set_bkg_tile_xy(index_X, index_Y, MOTHERSHIPS_BOTTOM_RIGHT_BOTTOM_LEFT_GREEN);  // MOTHERSHIPS_BOTTOM_RIGHT_BOTTOM_LEFT_GREEN
            set_bkg_tile_xy(index_X - 1, index_Y, MOTHERSHIP_BOTTOM_LEFT_GREEN);            // MOTHERSHIP_BOTTOM_LEFT_GREEN
            set_bkg_tile_xy(index_X - 1, index_Y - 1, MOTHERSHIP_TOP_LEFT_GREEN);           // MOTHERSHIP_TOP_LEFT_GREEN
            return 0x01U;
        } else if ((current_tile == MOTHERSHIPS_BOTTOM_RIGHT_GREEN_BOTTOM_LEFT) && (bulletx - tile_x <= 3)) {  // IF PLAYER HITS LEFT SIDE OF BOTH DARK GREEN MOTHERSHIPS BOTTOMS, WHILE RIGHT MOTHERSHIP IS GREEN
            set_bkg_tile_xy(index_X, index_Y - 1, MOTHERSHIP_TOPS_GREEN);                                      // MOTHERSHIP_TOPS_GREEN
            set_bkg_tile_xy(index_X, index_Y, MOTHERSHIPS_BOTTOMS_GREEN);                                      // MOTHERSHIPS_BOTTOMS_GREEN
            set_bkg_tile_xy(index_X - 1, index_Y, MOTHERSHIP_BOTTOM_LEFT_GREEN);                               // MOTHERSHIP_BOTTOM_LEFT_GREEN
            set_bkg_tile_xy(index_X - 1, index_Y - 1, MOTHERSHIP_TOP_LEFT_GREEN);                              // MOTHERSHIPS_BOTTOMS_GREEN
            return 0x01U;
        }
    }
